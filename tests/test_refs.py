...
def _wide_book() -> _Doc:
    """A book whose page 3 points at two three-page notes: six target pages."""
    contents = "\n".join(
        [
            "Contents",
            "2.1 ",
            "Impairment Provisions By Division",
            "5",
            "2.2 ",
            "Provisions for Impairment and Asset Quality",
            "8",
            "2.3 ",
            "Credit Impairment Overlays",
            "11",
        ]
    )
    pages = [
        ...
        "\n".join(
            [
                "Group Performance Analysis",
                FOOTER.format(printed=1),
                "Refer to Note 2.1 and Note 2.2 and Note 2.3 for the detail.",
            ]
        ),
        "spacer",
    ]
    for note_id, title in (
        ("2.1", "Impairment Provisions By Division"),
        ("2.2", "Provisions for Impairment and Asset Quality"),
        ("2.3", "Credit Impairment Overlays"),
    ):
        for part in range(3):
            suffix = " (continued)" if part else ""
            pages.append("\n".join(["Appendices", f"{note_id} ", f"{title}{suffix}", "rows"]))
    return _Doc("wide", pages)


def test_follow_adds_at_most_the_cap_in_new_pages():
    doc = _wide_book()
    followed = refs.follow_references({doc.doc_id: doc}, [(doc.doc_id, 3)], ["loan impairment provisions"])
    assert len(followed) == refs.MAX_FOLLOWED_PAGES == 4


def test_follow_tags_a_page_the_budget_already_holds_without_spending_the_cap():
    doc = _wide_book()
    budget = [(doc.doc_id, 3), (doc.doc_id, 5), (doc.doc_id, 6), (doc.doc_id, 7)]
    followed = refs.follow_references(
        doc_by_id={doc.doc_id: doc},
        selected=budget,
        relevance_terms=["loan impairment provisions credit overlays division asset quality"],
    )
    new = [f for f in followed if (f.doc_id, f.page) not in set(budget)]
    tagged = [f for f in followed if (f.doc_id, f.page) in set(budget)]
    assert len(new) == refs.MAX_FOLLOWED_PAGES
    assert {f.page for f in tagged} == {5, 6, 7}


def test_follow_ranks_a_note_reference_above_a_page_reference():
    doc = _book()
    followed = refs.follow_references(
        {doc.doc_id: doc},
        [(doc.doc_id, 4)],
        ["loan impairment provisions collective", "average interest earning assets"],
    )
    assert [(f.page, f.tier) for f in followed] == [(7, 0), (8, 0), (6, 1)]


def test_follow_drops_a_target_that_shares_nothing_with_the_case():
    doc = _book()
    assert refs.follow_references({doc.doc_id: doc}, [(doc.doc_id, 4)], ["cost to income ratio jaws"]) == []


def test_follow_is_stable_across_runs():
    doc = _wide_book()
    terms = ["loan impairment provisions"]
    first = refs.follow_references({doc.doc_id: doc}, [(doc.doc_id, 3)], terms)
    refs._notes_cache.clear()
    refs._printed_cache.clear()
    assert refs.follow_references({doc.doc_id: doc}, [(doc.doc_id, 3)], terms) == first


# --- provenance --------------------------------------------------------------


def test_provenance_names_the_source_page_and_the_target():
    doc = _book()
